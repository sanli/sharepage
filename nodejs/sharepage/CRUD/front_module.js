//#!本文件由share.js自动产生, 命令行为: node sharecli.js gen <$=module_name $>
<$
/**
...
<$ if(options.supportimport){ $>
        ...
        $('a.action-import-tmpl').on('click', function(e){
            e.preventDefault();
            window.location.href="/<$=options.module_desc $>表格（模板）.csv";
        });
<$ } $>
...
<$ } $>
    ...
    createModule: function(){
        function _createModule(condition, fn, fail){
            $M.doupdate('<$=options.url $>/create', condition, { successfn : fn , failfn: fail});
        };
...
<$ if(options.supportimport){ $>
    ...
    onImportFile : function(e){
        PG.doImportWithWebsocket('<$=module_name $>',{'title':'<$=options.module_desc $>导入'});
        e.preventDefault();
    },
<$ } $>
    // ========================================================================
    ...
    listNavi : function(fn, fail){
        $M.doquery('<$=options.url $>/listnavi'
            , { } 
    ...
    loadDataDetail : function(id, fn){
        $M.doquery('<$=options.url $>/get', {_id : id}
            , { successfn : fn , alertPosition : '#cellDiv'});
    }
