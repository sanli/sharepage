...
// 创建索引，以及设置唯一键
// TODO: 根据实际结构确定索引结构
<$=module_name $>.index({_id : 1})
    .index({module_name : 1}, { unique : true })
    .index({module_id : 1}, { unique : true });
var <$=module_name $> = mongoose.model('<$=module_name $>', <$=module_name $> ),
    _Module = <$=module_name $> ;
...
exports.importCSV = function(filename, fn, updater, opts) {
    ...
    var impfn = opts.verify 
      ? imp.createDataValidator(filename, cvsfield_validrule, extend({}, args, {
            ...
            enumname_prefix : '<$=options.module_desc $>'
        }), function(data){ 
    ...
    impfn.import(function(err, count, cellIds){
        if(err) {
            console.trace("导入<$=module_name $>出错", err);
            return fn(err)
        };
...
if(isme(__filename)){
  if(process.argv.length > 2 && isme(__filename)){
    ...
    console.log('<$=module_name $>db.js '+ testcmd.join('|'));
  }
}
