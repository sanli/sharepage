...
var Module = $.extend(new $M(), {
    ...
    bind : function(){
        ...
        $('a.action-import-tmpl').on('click', function(e){
            e.preventDefault();
            window.location.href="/数据字典管理表格（模板）.csv";
        });
        ...
        Module.listNavi(function(module){
            ...
            $('#navlist').treeview({
                ...
                //TODO : 触发相应的查询操作
                var enum_type = naviNode.data.value;
                var state = $.extend(true, {}, PG.state);
                state.cond.enum_type = enum_type;
                PG.pushState(state);
            });
        }); // ============= 实现数据导航功能 =============
    ...
    createModule: function(){
        ...
        $.showmodal('#moduleDlg', function(fn){
            if ($('#module-form').validate().form()){
                ...
                // ...
                data.items = Module.text2item(data.items);
    ...
    viewModule : function(id , options){
        ...
        Module.loadDataDetail(id, function(module){
            ...
            var itemtxt = Module.item2text(data.items);
            $('#module-form-view span[name=items]').text(itemtxt);
        });
    ...
    updateModule: function(id, options){
        ...
        Module.loadDataDetail(id, function(module){
            ...
            // TODO:autofill不能填充的数据在这里手工填充
            var itemtxt = Module.item2text(data.items);
            $('#module-form textarea[name=items]').val(itemtxt);
        });
    ...
    //处理导入数据
    onImportFile : function(e){
        PG.doImportWithWebsocket('enum',{'title':'数据字典导入'});
        e.preventDefault();
    },
    ...
    text2item : function(text){
        var dicts = text.split('\n');
        return _.map(dicts, function(dict){
            var v = dict.split(':');
            return { name : v[0] , value: v[1] }
        });
    },

    item2text : function(items){
        var is = _.map(items, function(item){   return item.name+':'+item.value});
        return is.join('\n');
    }
});
