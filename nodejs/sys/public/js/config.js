//#!本文件由share.js自动产生, 命令行为: node share.js gen config
...
var Module = $.extend(new $M(), {
    ...
    createModule: function(){
        function _createModule(condition, fn, fail){
            $M.doupdate('/web/sys/config/add.do', condition, { successfn : fn , failfn: fail});
        };
    ...
    deleteModule: function(id, options){
        $.ask("删除对象","是否确认删除,删除后不能恢复？", function(){
            $M.dodelete('/web/sys/config/delete.do'
                , { id : id }
                , { successfn : function(){
                        Module.loadPageData(PG.state.cond, PG.state.page);
                    }});
    ...
    updateModuleInfo : function(condition, fn, fail){
        $M.doupdate('/web/sys/config/update.do', condition, { successfn : fn , failfn: fail});
    },

    // 查询详细信息
    loadDataDetail : function(id, fn){
        $M.doquery('/web/sys/config/get.do', {id : id}
            , { successfn : fn , alertPosition : '#cellDiv'});
    }
